...
int main( void)
{
	...
	unsigned char   uValue1, uValue2, uValue3, uValue4;
	int      iValue1;
	char	 new_array[BUFF_SIZE];
	char	 new_array_payload[5];
	...
	while( 1 )
	{
		...
		else if (data.data_type == TYPE_CFG)
		{
			int CFG_HEADER_SZ = 8;
			memcpy(&uValue1, data.data_buff, sizeof(unsigned char));
			memcpy(&uValue2, data.data_buff+sizeof(unsigned char), sizeof(unsigned char));
			memcpy(&uValue3, data.data_buff+sizeof(unsigned char)*2, sizeof(unsigned char));
			memcpy(&uValue4, data.data_buff+sizeof(unsigned char)*3, sizeof(unsigned char));
			memcpy(&iValue1, data.data_buff+sizeof(int), sizeof(int));
			printf("Interpreted as 4 uchar and 1 integer : %02x, %02x, %02x, %02x, %d\n", uValue1, uValue2, uValue3, uValue4, iValue1);
			memcpy(new_array_payload, data.data_buff+CFG_HEADER_SZ, sizeof(char)*5);
			printf("Interpreted as array: ");
			int i;
			for(i = 0; i<iValue1; i++)
				printf("%c ", new_array_payload[i]);
			printf("\n");
		}
